...
int main() {
    ...
    while (lives > 0) {  // keep looping while lives remain
        ...
        lives = bee.update(input);   // update the game engine based on input    
...
void init() {
    ...
    lcd.setContrast(0.5);
    joystick.init();
    bee.init(0,8,2,2,2);     // paddle x position, paddle_height,paddle_width,ball_size,speed
}
...
void welcome() { // splash screen
    lcd.printString("     bee!    ",0,1); 
    lcd.printString("abcd",0,2); 
